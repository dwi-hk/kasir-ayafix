...
function switchTab(tabName) {
    const listTab = ['kasir', 'transaksi', 'master', 'titipan', 'pengeluaran', 'laporan'];
    listTab.forEach(tab => {
...
function renderMenu() {
    const container = document.getElementById('container-menu') || document.getElementById('containerMenuKasir');
    if (!container) return;

    const inputCari = document.getElementById('cariMenuKasir');
    const keyword = inputCari ? inputCari.value.toLowerCase().trim() : '';

    let items = typeof databaseMenu !== 'undefined' ? databaseMenu : [];
    ...
    if (keyword) {
        items = items.filter(m => m.nama.toLowerCase().includes(keyword));
    }
