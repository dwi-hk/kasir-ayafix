...
function switchSubMaster(sub) {
    ...
    if(sub === 'pelanggan') renderPelangganData();
}
...
/* ================= MANAJEMEN PELANGGAN & HUTANG (NODE: PELANGGAN) ================= */
function simpanPelanggan() {
    ...
    if (db) {
        db.ref('PELANGGAN/' + idPelanggan).set(dataBaru);
    } else {
        pelangganList.push(dataBaru);
...
function renderPelangganData() {
    let tbody = document.getElementById('tabelPelanggan');
    if (!tbody) return;

    let list = (db && dataPelangganFirebase.length > 0) ? dataPelangganFirebase : pelangganList;

    if (list.length === 0) {
        tbody.innerHTML = `<tr><td colspan="5" class="text-center py-4 text-gray-400">Belum ada data pelanggan di database</td></tr>`;
        return;
    }

    // Hitung akumulasi hutang berdasarkan transaksi kasir jika ada
    let mapHutangTransaksi = {};
    let sumberTx = (db && dataTransaksiFirebase.length > 0) ? dataTransaksiFirebase : riwayatTransaksi;
    
    sumberTx.forEach(t => {
        let metode = (t.metodePembayaran || t.metode || '').toUpperCase();
        if (metode === 'HUTANG' && t.pelangganId) {
            let nominal = parseNominalDinamis(t);
            if (!mapHutangTransaksi[t.pelangganId]) mapHutangTransaksi[t.pelangganId] = 0;
            mapHutangTransaksi[t.pelangganId] += nominal;
        }
    });

    let html = '';
    list.forEach(item => {
        let id = item.id || item.key || '-';
        let nama = item.nama || item.namaPelanggan || '-';
        let kontak = item.kontak || item.wa || item.noHP || item.telepon || '-';
        let alamat = item.alamat || item.lokasi || '-';
        
        // Menggunakan hutang dari node atau hasil kalkulasi transaksi
        let hutangNode = parseNominalDinamis(item);
        let totalHutang = hutangNode > 0 ? hutangNode : (mapHutangTransaksi[id] || 0);

        html += `
            <tr class="hover:bg-orange-50 border-b">
                <td class="p-2">
                    <p class="font-bold uppercase text-gray-800">${nama}</p>
                    <p class="text-[10px] font-mono text-gray-400">ID: ${id}</p>
                </td>
                <td class="p-2 font-mono text-xs">${kontak}</td>
                <td class="p-2 text-xs text-gray-600">${alamat}</td>
                <td class="p-2 text-right font-black ${totalHutang > 0 ? 'text-red-600' : 'text-emerald-600'}">
                    Rp ${totalHutang.toLocaleString('id-ID')}
                </td>
                <td class="p-2 text-center">
                    <button onclick="hapusPelanggan('${id}')" class="text-red-600 font-bold hover:underline">❌ Hapus</button>
                </td>
            </tr>
        `;
    });
...
function hapusPelanggan(id) {
    if (!confirm(`Hapus data pelanggan ${id}?`)) return;
    if (db) {
        db.ref('PELANGGAN/' + id).remove();
    } else {
        pelangganList = pelangganList.filter(p => (p.id || p.key) !== id);
        localStorage.setItem('aya_pelanggan', JSON.stringify(pelangganList));
        renderPelangganData();
    }
}
...
function parseNominalDinamis(exp) {
    if (!exp) return 0;
    let raw = exp.nominal ?? exp.jumlah ?? exp.total ?? exp.harga ?? exp.biaya ?? exp.hutang ?? 0;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if(db) {
        // Listener Realtime Node PELANGGAN
        db.ref('PELANGGAN').on('value', (snapshot) => {
            let data = snapshot.val();
            if (data) {
                dataPelangganFirebase = Object.keys(data).map(key => ({
                    key: key,
                    ...data[key]
                }));
            } else {
                dataPelangganFirebase = [];
            }
            renderPelangganData();
        });
